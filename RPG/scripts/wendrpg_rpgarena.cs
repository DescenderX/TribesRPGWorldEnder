...
$ArenaEquipment = "Hatchet 1 Knife 1 BoneClub 1 CastingBlade 1 ";
...
function CreateArenaStorage(%clientId)
{
	...
	for(%i = 0; %i < %max; %i++)
	{
		...
		if(%checkItemCount && %checkItem.className != Armor)
		{
			...
			storeData(%clientId, "BankStorage", rpg::ModifyItemList(fetchData(%clientId, "BankStorage"), %b, %checkItemCount));
	...
	if(%group != -1) {
		for(%i = 1; %i <= $maxroster; %i++) {
			if($ArenaRoster[%i] == "") {
				...
				%AIname = AI::helper($spawnIndex[floor(1+(getRandom() * 20))], "ArenaGladiator" @ %i, "MarkerSpawn " @ %marker);
