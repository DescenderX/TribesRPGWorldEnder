...
// DescX Note:		Doesn't really need a note. Tagged because this function is used, but its tiny and easy to miss.
function String::RemoveFromList(%list, %item, %sep) {
	%a = String::replace(%list, %sep @ %item, "");
	return String::replace(%a, %item @ %sep, "");
}
//_______________________________________________________________________________________________________________________________
function string::getBetween(%str, %leftSide, %rightSide) {
	%str = string::replace(%str, %leftSide, "");
	%str = string::replace(%str, %rightSide, "");
	return %str;
}
