		...
		if(%obj == "Player")
		{
			...
			if($BotInfo[%object.name, NAME] != "Weather Device")				newRotateTownBot(%clientId, %object, %clientPos, %botPos);			if(Client::getTeam(%clientId) != 0)				AI::sayLater(%clientId, %object, "Eek!", True);			else if(%closest <= ($maxAIdistVec + 5)){				if(%clientId.tmpbottalk != "")					%clientId.tmpbottalk = "";				endBotTalkChoice(%clientId);
				if($TheMandateDeliveryTarget[rpg::getName(%clientId)] == %object)
					...
									eval("bottalk::"@clipTrailingNumbers(%object.name)@"("@%clientId@","@%object@",True,\"#say hello\");");			}			else {
