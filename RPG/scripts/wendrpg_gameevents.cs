...
function RecursiveWorld(%seconds)
{
	...
	if(!$EndOfTheWorld || $EndOfTheWorld <= 0)
		$EndOfTheWorld = 0;

	if(rpg::IsTheWorldEnding() && $WorldEndEventAt > 0 && (getSimTime() > $WorldEndEventAt) ) {
		rpg::FinishWorldEnd();
	}

	if($ticker[1] >= (($SaveWorldFreq-15) / %seconds) && !$tmpNoticeSaveWorld)
	...
	if($ticker[1] >= ($SaveWorldFreq / %seconds)) {
		...
		for(%i = 0; GetWord(%list, %i) != -1; %i++) {
			...
			if(getWord(%vel, 2) <= -1000) {
				FellOffMap(%id);
				if(rpg::IsTheWorldEnding() && !Player::isAIControlled(%id))
					Player::Kill(%id);
			}
		}
		SaveWorld();
